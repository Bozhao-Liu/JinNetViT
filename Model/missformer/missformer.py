...
class MISSFormer(nn.Module):
    def __init__(self, in_channels=3, num_classes=1, token_mlp_mode="mix_skip", encoder_pretrained=True):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # 2. Resize to 224x224 (required for MISSFormer)
        x_resized = F.interpolate(x, size=(224,224), mode='bilinear', align_corners=False)

        # 3. Run normal MISSFormer pipeline
        encoder = self.backbone(x_resized)
        ...
        tmp_0 = self.decoder_0(tmp_1, bridge[0].permute(0,2,3,1))  # B,C,224,224

        # 4. Resize output back to original resolution
        out = F.interpolate(tmp_0, size=(H,W), mode='bilinear', align_corners=False)
