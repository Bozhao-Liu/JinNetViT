...
for variant in VARIANTS:
    ...
    df = pd.DataFrame(rows, columns=["Metric", "t-value", "p-value"])

    tex = df.to_latex(
        escape=False,
        index=False,
        caption=f"Paired $t$-tests comparing \\texttt{{jinppvit/{variant}}} vs \\texttt{{jinppvit/full}} on {DATASET_NAME}.",
        label=f"tab:ttest_{variant}_{DATASET_NAME}"
    )

    out_dir = os.path.join("Result", "ttest", variant)
    os.makedirs(out_dir, exist_ok=True)

    with open(os.path.join(out_dir, f"{DATASET_NAME}.tex"), "w") as f:
        f.write(tex)

    print(f"[saved] ttest/{variant}/{DATASET_NAME}.tex")
